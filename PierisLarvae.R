...
for(topt.k in 1:length(topts)){
  perf.mat[,topt.k]= sapply(dat.day$TALOC, FUN=beta_2012, a=tpc.beta[1],b=topts[topt.k],c=tpc.beta[3],d=tpc.beta[4],e=tpc.beta[5])
  #set NaN to zero
...
fig.fitnesscurves=ggplot(perfs.l, aes(x=temperature, y=performance, color=year, group=year))+geom_line()+
  ...
  xlab("thermal optima (C)")+ylab("growth rate (g/g/h)")+theme(legend.position = c(0.8, 0.8))+
  ylim(0.025,0.057)
